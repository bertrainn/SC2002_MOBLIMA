...
public class BookingMenu extends BaseMenu {
		...
		if (mov == null) {
			...
			for (MovieSession ms : movSess) {
				if (ms.getShownMovie().getShowingStatus() != Constants.SHOWING_STATUS.CS
						&& ms.getShownMovie().getTitle().equals(mov.getTitle()))
					curMs.add(ms);
			}
			if (curMs.isEmpty()) {
	...
	/**
	 * Shows the Booking Confirmation Menu by Movie that will be loaded into the
	 * load method.
	 */
	...
	private void showMovies(ArrayList<MovieSession> curMs) {
		...
		for (MovieSession ms : curMs) {
			...
			printMenuWithoutSpace(++i + ". " + reduceStringLength(cinp.getName(), 14)
					+ generateSpaces(15 - cinp.getName().length())
